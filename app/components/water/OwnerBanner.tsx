...
export function OwnerBanner({ record, registered }: OwnerBannerProps) {
  ...
  return (
    <motion.div
      className="fixed top-4 right-4 z-[90] pointer-events-auto"
      initial={{ opacity: 0, y: -10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, ease: "easeOut" }}
    >
      <div className="rounded-xl border border-sky-500/30 bg-slate-950/75 px-2.5 py-1.5 shadow-xl shadow-sky-900/40 backdrop-blur">
        <div className="flex flex-col gap-1.5 text-[10px] sm:text-[11px]">
          {/* First Row: Owner Name */}
          <div className="flex items-center gap-1.5 justify-center">
            <span className="font-semibold text-sky-50/95 truncate max-w-[180px] sm:max-w-[220px]">
